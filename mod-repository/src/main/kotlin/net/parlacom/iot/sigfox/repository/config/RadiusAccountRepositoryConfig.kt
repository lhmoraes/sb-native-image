...
import net.parlacom.iot.sigfox.repository.config.RepositoryConstant.RADIUS_ACCOUNT_CONNECTION_FACTORY
import net.parlacom.iot.sigfox.repository.config.RepositoryConstant.RADIUS_ACCOUNT_TEMPLATE
 ...
 */
@Configuration
@EnableR2dbcRepositories(entityOperationsRef = RADIUS_ACCOUNT_TEMPLATE,
                         basePackages = ["net.parlacom.iot.sigfox.repository.account"])
class RadiusAccountRepositoryConfig {

    @Value("\${sigfox.datasource.account.r2dbc}")
     ...
     */
    @Bean
    @Qualifier(value = RADIUS_ACCOUNT_CONNECTION_FACTORY)
    fun radiusAccountConnectionFactory(): ConnectionFactory {
     ...
     */
    @Bean
    fun radiusAccountEntityTemplate(@Qualifier(RADIUS_ACCOUNT_CONNECTION_FACTORY) connectionFactory: ConnectionFactory): R2dbcEntityOperations {
     ...
     * databasePopulator.populate(radiusAccountConnectionFactory()).subscribe()
     */
    @PostConstruct
    fun initialize() {
        val databasePopulator = ResourceDatabasePopulator()
